...
function product_adder_theme_check_notice() {
	if ( is_integration_mode_selected() && get_integration_type() == 'simple' ) {
		...
		<div id="implecode_message" class="updated product-adder-message messages-connect">
			<div class="squeezer">
				<h4><?php echo sprintf( __( 'You are currently using %s in Simple Mode. It is perfectly fine to use it this way, however some features are limited. To switch to Advanced Mode you probably need Theme Integration.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ); ?></h4><h4><?php echo sprintf( __( 'You can also use a fully compatible %sCatalog Me! theme%s.', 'post-type-x' ), '<a href="' . admin_url( 'theme-install.php?search=Catalog%20me' ) . '">', '</a>' ) ?></h4>
		...
		<div id="implecode_message" class="error product-adder-message messages-connect">
			<div class="squeezer">
				<h4><?php echo sprintf( __( '<strong>Your theme does not declare %s support</strong> &#8211; please proceed to sample product page where automatic layout adjustment can be done.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) ?> <?php echo sprintf( __( 'You can also use a fully compatible %sCatalog Me! theme%s.', 'post-type-x' ), '<a href="' . admin_url( 'theme-install.php?search=Catalog%20me' ) . '">', '</a>' ) ?></h4>
				<p class="submit">
					<?php echo sample_product_button() ?>
					<a href="https://implecode.com/docs/post-type-x/core/theme-integration-wizard/#cam=default-mode&key=top-message-video" class="button"><?php _e( 'Theme Integration Video', 'post-type-x' ); ?></a>
...
function implecode_plugin_review_notice() {
	...
	$text = apply_filters( 'ic_review_notice_text', sprintf( __( '%s is a free software. Would you mind taking <strong>5 seconds</strong> to <a target="_blank" href="%s">rate the plugin</a> for us please? Your comments <strong>help others know what to expect</strong> when they install %s.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://wordpress.org/support/view/plugin-reviews/ecommerce-product-catalog#postform', IC_CATALOG_PLUGIN_NAME ) )
	?>
