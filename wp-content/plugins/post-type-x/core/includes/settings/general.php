				...
				<form method="post" action="options.php">
					...
					if ( !is_advanced_mode_forced() || ic_is_woo_template_available() ) {
						...
						if ( ic_is_woo_template_available() ) {
							echo '<p>' . __( 'If you have any problems with catalog layout you can use the theme integration to fix it.', 'post-type-x' ) . '</p>';
						}
						...
						if ( is_integration_mode_selected() ) {
							...
							<table>
								<?php
								implecode_settings_radio( __( 'Choose theme integration type', 'post-type-x' ), 'archive_multiple_settings[integration_type][' . $theme . ']', $archive_multiple_settings[ 'integration_type' ][ $theme ], array( 'simple' => __( 'Simple Integration', 'post-type-x' ), 'advanced' => __( 'Advanced Integration', 'post-type-x' ) ) );
								?></table>
					...
					<table>
						<tr>
							<td style="width: 180px">
								<?php _e( 'Enable Main Listing Page', 'post-type-x' ); ?>:
							</td>
							<td>
								<input title="<?php _e( 'Disable and use [show_products] shortcode to display the products.', 'post-type-x' ); ?>" type="checkbox" name="enable_product_listing" value="1"<?php checked( 1, $enable_product_listing ); ?> />
							</td>
						</tr>
						<tr>
							<td>
						  ...
						  <tr>
						  <td><?php _e('Product listing URL', 'post-type-x'); ?>:</td>
						...
						<tr>
							<td><?php _e( 'Listing shows at most', 'post-type-x' ); ?> </td>
							<td><input
					...
					<table>
						<?php if ( is_ic_permalink_product_catalog() ) { ?>
							<tr>
								...
								<td class="longer"><?php echo $site_url ?>/<input <?php echo $disabled ?> type="text"
																										  ...
																										  title="<?php _e( 'Cannot be the same as product listing page slug.', 'post-type-x' ) ?>"
						...
						implecode_settings_radio( __( 'Category Page shows', 'post-type-x' ), 'archive_multiple_settings[category_top_cats]', $archive_multiple_settings[ 'category_top_cats' ], array( 'off' => $uc_item_name, 'on' => $uc_item_name . ' & ' . __( 'Subcategories', 'post-type-x' ), 'only_subcategories' => __( 'Subcategories', 'post-type-x' ) ) );
						implecode_settings_radio( __( 'Categories Display', 'post-type-x' ), 'archive_multiple_settings[cat_template]', $archive_multiple_settings[ 'cat_template' ], array( 'template' => __( 'Template', 'post-type-x' ), 'link' => __( 'URLs', 'post-type-x' ) ), true, array( 'template' => __( 'Display categories with the same listing theme as products.', 'post-type-x' ), 'link' => __( 'Display categories as simple links.', 'post-type-x' ) ) );
						...
						implecode_settings_radio( __( 'Show Related', 'post-type-x' ), 'archive_multiple_settings[related]', $archive_multiple_settings[ 'related' ], array( 'products' => $uc_item_name, 'categories' => __( 'Categories', 'post-type-x' ), 'none' => __( 'Nothing', 'post-type-x' ) ) );
					...
					<table>
						<tr>
							<td><?php _e( 'Enable Catalog Breadcrumbs:', 'post-type-x' ); ?> </td>
