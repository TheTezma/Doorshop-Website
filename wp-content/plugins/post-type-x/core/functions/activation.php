...
function create_sample_product() {
	...
	if ( current_user_can( 'publish_products' ) && ((!is_advanced_mode_forced() && empty( $sample_id )) || isset( $_GET[ 'create_sample_product_page' ] )) ) {
		...
		$product_field[ 'excerpt' ] .= '<p><strong>' . __( 'Please read this page carefully to fully understand the difference between simple and advanced mode and how the product page looks like.', 'post-type-x' ) . '</strong></p>';

		$long_desc					 = '<p>' . __( 'This section is product long description. It should appear under the attributes table. Between the short description and the attributes table you should see the price, SKU and shipping options (all can be disabled). The attributes also can be disabled.', 'post-type-x' ) . '</p>';
		...
		$long_desc .= '<p>' . __( 'Switching to Advanced Mode also gives additional features: automatic product listing, category pages, product search and category widget. Building a product catalog in Advanced Mode will be less time consuming as you don\'t need to use a shortcode for everything.', 'post-type-x' ) . '</p>';
		...
		$long_desc .= '<h2>' . __( 'Product Description End', 'post-type-x' ) . '</h2>';
		$long_desc .= '<p>' . __( 'Below the product description you should see nothing apart of return to products URL and Advanced Mode Test which will not show up on your product pages. When using advanced mode also the related products will show up.', 'post-type-x' ) . '</p>';
		$long_desc .= '<p>' . sprintf( __( 'Thank you for choosing %s. If you have any questions or comments please use <a target="_blank" href="%s">plugin support forum</a>.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://wordpress.org/support/plugin/ecommerce-product-catalog' ) . '</p>';
