...
/*
  ...
  Plugin URI: http://demo.nimble3.com/nimble-portfolio/
...
if (!class_exists('NimblePortfolioPlugin')) {
    ...
    class NimblePortfolioPlugin {
        ...
        static function init($params = array()) {
            ...
            // Beta version notification in admin Dashboard
            add_action('wp_ajax_nimble_portfolio_beta_version3_notice', array(__CLASS__, 'adminNoticeBetaVersion3Callback'));
            if (get_option("nimble_portfolio_beta_version3_notice", false) == false) {
                add_action('admin_notices', array(__CLASS__, 'adminNoticeBetaVersion3')); 
            }
        ...
        static function adminNoticeBetaVersion3() {
            ?>
            <div class="notice notice-info is-dismissible" id="nimble_portfolio_beta_version3_notice">
                <h3>Nimble Portfolio</h3>
                <p>We are proud to release <strong>Version 3.0.0-BETA</strong> after receiving an overwhelming response from our customers. The new version includes several new improvements as below.</p>
                <ol>
                    <li>Default Skin is upgraded with pro features. (Premium Default+ Skin becomes Default skin)</li>
                    <li>PrettyPhoto Lightbox is upgraded with pro features. (Premium PrettyPhoto Lightbox becomes Default lightbox)</li>
                    <li>Redux Framework is used for Options Management.</li>
                    <li>Plugin is Translation ready now.</li>
                    <li>W3C validated.</li>
                </ol>
                <p>You are welcome to <a href="http://bit.ly/wp-gallery-3-beta">Download</a> and test drive the new version and report us any issues you come across through the wordpress support forum.</p>
                <p>Thank you all.</p>
            </div>
            <?php
        }

        static function adminNoticeBetaVersion3Callback() {
            update_option("nimble_portfolio_beta_version3_notice", true);
        }
